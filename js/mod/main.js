function renderVersion(data, config) {
    ...
    const modPath = config?.imagePath || '../img/foreverbox/';

    const newIcon = version.meta.isNew ? createNewIcon() : '';
    const modioIcon = version.meta.isModIo ? createModIoIcon(version.meta.isNew) : '';
    const incredimodsIcon = version.meta.isApproved ? createIncrediModsIcon(version.meta.isNew, version.meta.isModIo) : '';
        ...
        <div class="version-container" style="background-color: ${version.visu.colCont};" 
            ...
            <div class="version-content">
                <div class="version-image">
                    <img src="${modPath}splashes/${version.meta.name.toLowerCase().replace(/ /g, '-')}.jpg" alt="${version.meta.name}">
                </div>
                <div class="version-info" data-preview-id="${version.meta.previewID || ''}">
                    <div class="version-name-container">
                        ${newIcon}
                        ${modioIcon}
                        ${incredimodsIcon}
                        <div class="version-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="10 0 100 100"
    ...
    setupVersionButtons(container);

    if (data.characters) {
        renderCharacters(data.characters, data.versionInfo.meta.version, config);
    } else {
...
function createNewIcon() {
    return `
        <div class="version-icon-top-right">
            <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 100 100" fill="#00ff00">
                <path d="M79.162 75.367q-4.04 0.495 -4.655 4.519 -0.615 4.022 -4.611 3.247t-5.823 2.859q-1.829 3.635 -5.389 1.663 -3.559 -1.972 -6.423 0.92 -2.862 2.893 -5.638-0.084 -2.775-2.977 -6.391-1.109 -3.616 1.866 -5.336-1.822 -1.72-3.689 -5.736-3.031 -4.016 0.657 -4.513-3.381 -0.496-4.04 -4.519-4.655 -4.023-0.616 -3.247-4.611 0.777-3.994 -2.859-5.824 -3.635-1.827 -1.663-5.387 1.973-3.56 -0.92-6.422 -2.893-2.864 0.084-5.639 2.977-2.775 1.11-6.391-1.867 -3.617 1.821 -5.336 3.688 -1.72 3.03 -5.737-0.657 -4.016 3.382 -4.512 4.039 -0.496 4.655 -4.519 0.615-4.023 4.61-3.247 3.995 0.777 5.824-2.859 1.828-3.636 5.388-1.663t6.422-0.919c1.908-1.93 3.788-1.901 5.638 0.083q2.776 2.976 6.392 1.111 3.616-1.867 5.337 1.821 1.72 3.688 5.736 3.03 4.016-0.658 4.512 3.381 0.497 4.039 4.519 4.654t3.247 4.611q-0.776 3.996 2.859 5.824 3.636 1.829 1.663 5.388 -1.972 3.56 0.92 6.423 2.892 2.863 -0.085 5.638c-2.977 2.775 -2.354 3.981 -1.109 6.392q1.866 3.615 -1.823 5.337 -3.688 1.72 -3.03 5.736c0.439 2.679 -0.687 4.181 -3.38 4.513" />
                <text x="50%" y="52%" text-anchor="middle" fill="white" font-size="30px" font-family="Urbanist" font-weight="bold" dy=".3em">NEW</text>
            </svg>
        </div>
    `;
}

function createModIoIcon(isNew) {
    const style = isNew ? 'style="right: 60px;"' : '';
    return `
        <div class="version-icon-top-right" ${style}>
            <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0
                    <path d='m43.9 18.5c-.3.1-.6.3-.9.4-.7.2-1.4.3-2.1.3-3.6 0-6.6-2.9-6.6-6.6v-.1c-1.4-.3-2.8-.5-4.3-.5-1.1 0-2.1.1-3.1.3-.1 3.5-3 6.3-6.6 6.3-1.2 0-2.4-.3-3.4-.9-1.7 1.8-3.1 4-3.9 6.5 1.8 1.2 3 3.2 3 5.5 0 2.4-1.3 4.5-3.2 5.6.7 2.3 1.9 4.4 3.4 6.2 3-1.6 5.3-4.2 6.5-7.5.5-1.5.8-3 .8-4.7 0-1.1-.1-2.2-.4-3.2 2.9-.6 5.5-2 7.4-4.1 1.8 2 4.3 3.4 7 4h.1c.9.2 1.9.3 2.9.3 2.3 0 4.5-.6 6.4-1.6.1 0 .1-.1.2-.1-.6-2.2-1.7-4.3-3.2-6.1z' />
                </svg>
            </svg>
        </div>
    `;
...
function createIncrediModsIcon(isNew, isModIo) {
    let style = '';
    if (isNew) style = 'right: 110px;';
    else if (isModIo) style = 'right: 60px;';
    return `<div class="version-icon-top-right" style="${style}" id="incredimods"></div>`;
}

function setupVersionButtons(container) {
    ...
    const versionInfo = versionContainer.querySelector('.version-info');
    ...
    const previewID = versionInfo?.getAttribute('data-preview-id');
    ...
    if (downloadBtn) {
        downloadBtn.addEventListener('click', () => {
            ...
            windowsLink.style.display = downloadWindowsUrl ? 'inline-block' : 'none';
            windowsLink.href = downloadWindowsUrl || '#';

            androidLink.style.display = downloadAndroidUrl ? 'inline-block' : 'none';
            androidLink.href = downloadAndroidUrl || '#';

            modIoLink.style.display = downloadModIoUrl ? 'inline-block' : 'none';
    ...
    if (previewBtn && previewID) {
        previewBtn.addEventListener('click', () => {
            const modal = document.getElementById('previewModal');
            const iframe = document.getElementById('previewVideo');
            iframe.src = `https://www.youtube.com/embed/${previewID}?autoplay=1`;
...
function renderCharacters(characters, versionId, config) {
    ...
    const slidesHtml = characters.map((char, i) => {
        ...
        <div class="slide" data-index="${i}" style="display:${i === 0 ? 'flex' : 'none'}">
            <div class="slide-img">
                <img src="${modPath}characters/${appearances[0]?.img || ''}" alt="${char.name}">
            </div>
    ...
    setupCharacterSlideshow(grid, characters);
}

function setupCharacterSlideshow(grid, characters) {
    let currentIndex = 0;
    ...
    function showSlide(idx) {
        ...
        currentIndex = idx;
        updateActiveAppearance(slides[idx], characters[idx]);
    }

    function updateActiveAppearance(slide, character) {
        const imgEl = slide.querySelector('.slide-img img');
        const btns = slide.querySelectorAll('.appearance-btn');
        if (btns && btns.length > 0) {
            ...
            btns.forEach(b => {
                ...
                const appearances = character.imgs || [];
    ...
    grid.querySelectorAll('.appearance-btn').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            if (selected) {
                const modPath = imgEl.src.split('/characters/')[0] + '/characters/';
                imgEl.src = `${modPath}${selected.img}`;
...
function removeVersionHr() {
    document.getElementById('version-hr')?.remove();
    document.getElementById('version-br')?.remove();
}
...
window.onclick = function (event) {
    const modals = {
        ...
        'blankIconModal': document.getElementById('blankIconModal')
    };
